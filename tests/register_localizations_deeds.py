...
pip install astropy numpy simpleITK
...
def read_deformation_field(file_path):
    """Read a deformation field from a TIF file."""
...
def main():
    ...
    regex_pattern = r'scan_(?P<runNumber>[0-9]+)_(?P<cycle>RT[0-9]+)_(?P<roi>[0-9]+)_ROI_converted_decon_(?P<channel>ch00)_DF.tif'
    ...
    if len(deformation_files)==0:
        ...
        print(f"$ Found this files to process: {deformation_files}")
    ...
    for deformation_file in deformation_files:
        ...
        if match:
            ...
            print(f"$ Analysing file: {deformation_file}")
            ...
            if channel == 'ch00' and 'RT' in cycle:
                ...
                #deformation_field_file = deformation_file_path.replace('.tif', '_DF.tif')
                deformation_field_file = deformation_file
                
                if os.path.exists(deformation_field_file):
                    print(f"Applying deformation field: {deformation_field_file} for barcode ID: {barcode_id}")
                    barcode_localizations = localizations[localizations['Barcode #'] == barcode_id]
                    deformation_field = read_deformation_field(deformation_field_file)
                    ...
                    print(f"Deformation field not found: {deformation_field_file}")
