...
def parseArguments():
    ...
    parser.add_argument("--input", help="Name of input mask file (TIFF, NPY, or HDF5 format).")
    ...
    parser.add_argument("--pyHiM", help="Will shift and interpolate intensity images", action="store_true")
    parser.add_argument("--zBinning", default = 2, type = int, help="zBinning used for mask segmentation")
    ...
    if args.pipe:
        ...
        pipe_status = False
        if not args.input:
            parser.error("mask_filter.py: error: the following arguments are required: --input. Otherwise provide images using --pipe! ")
...
def main():
    ...
    for file in files:
        ...
        if pipe_status:
            if args.pyHiM:
                file_image = file
                file = file.replace('.npy', '_3Dmasks.npy').replace('.tif', '_3Dmasks.npy').replace('.h5', '_3Dmasks.npy')
                file = os.path.join(os.path.dirname(file), 'mask_3d/data/', os.path.basename(file))
            else:
                file_image = []
        ...
        if args.replace_mask_file:
            ...
            if args.output is not None:
                save_path = file.replace('.tif','_'+args_output+'.tif').replace('.npy', '_'+args_output+'.npy').replace('.h5', '_'+args_output+'.h5')
            else:
